...
if __name__ == "__main__":
    datasets = {
        'laser': (getDataLaser, [nrmse, r_squared], True),
        'dde': (getDataDDE, [nrmse, r_squared], True),
        'lorenz': (getDataLorenz, [nrmse, r_squared], True),
        'mgs': (getDataMGS, [nrmse, r_squared], True),
        'sunspots': (getDataSunspots, [nrmse_sunspots, r_squared], False),
        'water': (getDataWater, [nrmse, r_squared], False),
    }
    dataset = sys.argv[2]
    dataLoader, errorMetrics, isAutoregressive = datasets[dataset]

    runBOExperiment(dataset, dataLoader, errorMetrics, isAutoregressive)
