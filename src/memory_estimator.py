...
def estimate_lms_memory(input_dim, output_dim):
    return output_dim * (
        0.00003302808602650960234375 * input_dim + 0.0012229283650716145833
    )
...
def estimate_ridge_memory(output_dim, input_shape):
    ...
    total_size = (
        ...
        + bias_size
        + buffer_size
        + max(backward_size, partial_backward_size)
    )
...
def estimate_nvar_memory(delay, order, input_shape):
    ...
    idx_memory = (
        comb(effective_linear_dim + order - 1, order, exact=True) * order * float_size
    )
...
def estimate_reservoir_memory(units, input_shape):
    ...
    memory += (
        input_shape[0] * input_shape[1] * float_size
    )  # u, multiplied by batch_size
    memory += (
        input_shape[0] * units * float_size
    )  # r, assuming r is retained for each sample

    # Assuming noise vectors are the same size as input and state vectors
    memory += (
        input_shape[0] * input_shape[1] * float_size
    )  # noise for u, multiplied by batch_size
    memory += (
        input_shape[0] * units * float_size
    )  # noise for r (state noise), multiplied by batch_size
...
def getInputDimension(architecture, idx):
    ...
    for edge in architecture["edges"]:
        ...
        if inputNode["type"] == "Input":
            ...
            nonlinear_dim = comb(
                effective_linear_dim + inputNode["params"]["order"] - 1,
                inputNode["params"]["order"],
                exact=True,
            )
            outputDim = effective_linear_dim + nonlinear_dim
        elif (
            inputNode["type"] == "Ridge"
            or inputNode["type"] == "RLS"
            or inputNode["type"] == "LMS"
        ):
...
def estimateMemory(architecture, numInputs):
    ...
    for i, node in enumerate(architecture["nodes"]):
        ...
        if node["type"] == "LMS":
            ...
            memory = estimate_ridge_memory(
                node["params"]["output_dim"], [numInputs, inputDimension]
            )
        elif node["type"] == "NVAR":
            memory = estimate_nvar_memory(
                node["params"]["delay"],
                node["params"]["order"],
                [numInputs, inputDimension],
            )
        elif node["type"] == "Reservoir" or node["type"] == "IPReservoir":
            memory = estimate_reservoir_memory(
                node["params"]["units"], [numInputs, inputDimension]
            )
