...
from reservoirpy.activationsfunc import get_function as get_function_np
from reservoirpy.mat_gen import bernoulli as bernoulli_np, uniform as uniform_np
from reservoirpy.node import Node as NodeNP
...
# Non-JAX version
# ---------------------------------------------------------------------------
class IPReservoir(NodeNP):
    """Drop-in replacement for ``reservoirpy.nodes.IPReservoir`` that works
    correctly inside a ``Model`` pipeline.
    ...
    You can also call ``train_ip(x, warmup)`` explicitly before building or
    fitting the model.
    ...
    def __init__(
        ...
        Win: Union[Weights, Callable] = bernoulli_np,
        W: Union[Weights, Callable] = uniform_np,
        bias: Union[Weights, Callable] = bernoulli_np,
        ...
        self.W = W
        self.bias = bias
        self.activation = get_function_np(activation)
    ...
    def initialize(self, x, y=None):
        ...
        self.state = dict(
            internal=np.zeros((self.output_dim,)),
    ...
    def train_ip(self, x: NodeInput, warmup: int = 0) -> "IPReservoirFixed":
