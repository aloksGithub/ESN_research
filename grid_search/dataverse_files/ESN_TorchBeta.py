...
"""
Forked from ReservoirPy and adapted to pytorch by @jviehweg
for use in:
https://doi.org/10.1016/j.neucom.2022.11.044

Allows specifically for a speed-up for testing multiple values for \beta without calculating whole state space for S_T time steps again
...
class ESN():
    ...
    def autocheck_nan(self):
        ...
        if torch.isnan(torch.from_numpy(self.W).byte().any()) == False:
            return 0
    ...
    def train(self, inputs, teachers, wash_nr_time_step, reset_state=True, verbose=False):
        ...
        for (j, (inp, tea)) in enumerate(zip(inputs, teachers)):
            ...
            self.Win = torch.from_numpy(self.Win)
            ...
            for t in range(inp.shape[0]): # for each time step in the input
                ...
                if self.Wfb is None:
                    ...
                    xdot = torch.matmul( self.W, x )
                    udot = torch.matmul( self.Win, u[t,:].reshape(di,1) )
                    x = ((1-self.lr) * x  +  self.lr * torch.tanh( udot + xdot ))#.astype(self.typefloat)
                    if verbose:
                        print("DEBUG AFTER")
                    ...
                    print('W: ', self.Win.shape)
                    print('u: ', u[t,:].reshape(di,1).shape)
                    
                    x = ((1-self.lr) * x  +  self.lr * torch.tanh( torch.matmul( self.Win, u[t,:].reshape(di,1) ) + torch.dot( self.W, x ) + torch.dot( self.Wfb, self.fbfunc(y) ) )).astype(self.typefloat)
        ...
        # train the output
        X_T = torch.transpose(X,0,1) # dim of X_T (nr of time steps, nr of neurons)
        self.X_T = X_T
    ...
    def WoutCalc(self, ridge):
        #X = self.X.numpy().copy()
        #X = torch.from_numpy(X)
        #Y = self.Y.numpy().copy()
        #Y = torch.from_numpy(Y)
        #X_T = self.X_T.numpy().copy()
        ...
        ridgeid = (ridge*torch.eye(1+self.N)).type(torch.DoubleTensor)#.astype(self.typefloat)
        #print(Y.type(), ' ridge: ', ridgeid.type(), ' x: ', X.type(), ' ', X_T.type())
        Wout = torch.matmul(torch.matmul(Y,X_T), torch.inverse(torch.matmul(X,X_T) + ridgeid ) )
        self.Wout = Wout
        #print(Wout)
        return Wout.numpy()
        if verbose:
        ...
        if self.reg_model is not None:
            ...
            #print('N: ', self.N, 'ridge: ', self.ridge)
            #brekn()
            #Y = torch.from_numpy(Y).type(torch.DoubleTensor)
            ridgeid = (self.ridge*torch.eye(1+self.N)).type(torch.DoubleTensor)#.astype(self.typefloat)
            #print(Y.type(), ' ridge: ', ridgeid.type(), ' x: ', X.type(), ' ', X_T.type())
            Wout = torch.matmul(torch.matmul(Y,X_T), torch.inverse(torch.matmul(X,X_T) + \
                    ridgeid ) )
        ...
        #        str(np.mean(Wout))+"\n\tnumpy_mean_Wout="+str(np.mean(np_Wout)))
            ...
            Wout = torch.dot( Y, torch.pinverse(X) )
    ...
    def run(self, inputs, reset_state=True, verbose=False):
        ...
        if reset_state:
            x = torch.zeros((self.N,1),dtype=self.typefloat)
            if self.Wfb is not None:
                y = torch.zeros((self.dim_out,1),dtype=self.typefloat)
        else:
        ...
        for (j, inp) in enumerate(inputs):
            ...
            # reset the states or not, that is the question
            if reset_state:
                x = torch.zeros((self.N,1),dtype=self.typefloat)#.astype(self.typefloat) # current internal state initialized at zero
                if self.Wfb is not None:
                    y = torch.zeros((self.dim_out,1),dtype=self.typefloat)

            all_int_states[j] = np.zeros((self.N,inp.shape[0]),dtype=self.typefloat)#torch.from_numpy(np.zeros((self.N,inp.shape[0]),dtype=self.typefloat))
            all_outputs[j] = np.zeros((self.Wout.shape[0],inp.shape[0]),dtype=self.typefloat)#torch.from_numpy(np.zeros((self.Wout.shape[0],inp.shape[0]),dtype=self.typefloat))
            ...
            u = u[0,:].reshape(di,1)
            u = torch.from_numpy(u)
            one = torch.reshape(torch.tensor([1]), (1,1)).type(torch.DoubleTensor)
            for t in range(inp.shape[0]): # for each time step in the input
                # x = (1-a)*x + a*np.tanh( np.dot( Win, np.vstack((1,u)) ) + np.dot( W, x ) )
                if self.Wfb is None:
                    x = (1-self.lr) * x  +  self.lr * torch.tanh( torch.matmul( self.Win, u ) + torch.matmul( self.W, x ) )
                else:
                    x = (1-self.lr) * x  +  self.lr * np.tanh( np.matmul( self.Win, u ) + np.matmul( self.W, x ) + np.dot( self.Wfb, self.fbfunc(y) ) )
                # y = np.dot( Wout, np.vstack((1,u,x)) )

                #Ersetzen von u mit y zum Aufakkumulieren des Fehlers.
                #print(one.type(), ' x: ', x.type())
                y = torch.matmul( self.Wout, torch.cat((one,x), dim=0) )#.astype(self.typefloat)
                u = y
                u = torch.cat((one,u), dim=0)
                u = torch.reshape(u,(di, 1))
                ...
                # u = data[trainLen+t+1]
                if verbose:
                    print("x.reshape(-1,).shape", x.view(-1,).size().shape)
                    print("y.reshape(-1,).shape", y.view(-1,).size().shape)
                    print("all_int_states[j][:,t].shape", all_int_states[j][:,t].shape)
                    print("all_outputs[j][:,t].shape", all_outputs[j][:,t].shape)
                #print(all_int_states[j][:,t].size(), ' ', all_outputs[j][:,t].Size())
                x2 = x.numpy()
                y2 = y.numpy()
                #print(y2)
                #print(y)
                all_int_states[j][:,t] = x2.reshape(-1,)
                all_outputs[j][:,t] = y2.reshape(-1,)
